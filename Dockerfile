...
RUN apk -U add git
WORKDIR /go/src/github.com/helm/chart-testing/v3/
COPY . .
...
RUN curl -LO "https://get.helm.sh/helm-$helm_version-linux-arm64.tar.gz" && \
    ...
    tar -xzf "helm-$helm_version-linux-arm64.tar.gz" -C "/usr/local/helm-$helm_version" && \
    ln -s "/usr/local/helm-$helm_version/linux-arm64/helm" /usr/local/bin/helm && \
    rm -f "helm-$helm_version-linux-arm64.tar.gz"

COPY ./etc/chart_schema.yaml /etc/ct/chart_schema.yaml
COPY ./etc/lintconf.yaml /etc/ct/lintconf.yaml
COPY --from=builder /go/bin/ct /usr/local/bin/ct
